/**
 * Jester King pilot — Spacecrafted events calendar
 *
 * Usage: node scripts/pilot-jester-king.mjs [--probe-only]
 */
...
const SLUG = "jester-king";
const CALENDAR = "https://jesterkingbrewery.com/events-calendar";
const WORKER =
...
console.log("\n=== Jester King pilot (Spacecrafted) ===\n");
...
if (vErr || !venue) {
	const { data: alt } = await sb
		.from("venues")
		.select("id, slug, name")
		.or("slug.ilike.%jester%,name.ilike.%jester%")
		.limit(8);
...
await sb
	...
	.update({
		...
		website_url: venue.website_url || "https://jesterkingbrewery.com/",
...
if (sourceId) {
	...
	if (error) {
		// Constraint may not allow spacecrafted yet — fall back to auto (HTML detect)
		const { error: e2 } = await sb
			.from("venue_event_sources")
			.update({ ...sourcePayload, platform_type: "auto" })
			.eq("id", sourceId);
		if (e2) throw new Error(`source update: ${error.message}; fallback: ${e2.message}`);
		console.log("Updated source", sourceId, "→ auto / draft / 90d (spacecrafted constraint missing)");
	} else {
		console.log("Updated source", sourceId, "→ spacecrafted / draft / 90d");
	}
} else {
...
const { count: before } = await sb
	...
	.eq("venue_id", venue.id);
console.log("\nPending for Jester King before:", before ?? 0);
...
const { data: recent } = await sb
	.from("ingested_events")
	.select("id, raw_title, parsed_starts_at, review_status, match_status, image_url, ticket_url")
	.eq("venue_id", venue.id)
	.order("created_at", { ascending: false })
	.limit(25);

console.log("\nPending for Jester King after:", after ?? 0);
...
console.log("\n=== Jester King pilot done (draft mode) ===");
