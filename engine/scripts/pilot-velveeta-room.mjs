/**
 * The Velveeta Room pilot — SeatEngine calendar + draft ingest.
 * Usage: node scripts/pilot-velveeta-room.mjs [--probe-only]
 */
...
const SLUG = "the-velveeta-room";
const CALENDAR = "https://the-velveeta-room-the-velveeta-room.seatengine.com/calendar";
const WEBSITE = "https://www.thevelveetaroom.com/";
const WORKER =
...
console.log("\n=== The Velveeta Room pilot (SeatEngine) ===\n");
...
if (sourceId) {
	...
	if (error) {
		...
		console.log("Updated source", sourceId, "→ auto / draft / 120d");
	} else {
...
const { count: before } = await sb
	...
	.eq("venue_id", venue.id);
console.log("\nPending for Velveeta before:", before ?? 0);
...
const { data: recent } = await sb
	.from("ingested_events")
	.select("id, raw_title, parsed_starts_at, review_status, match_status")
	.eq("venue_id", venue.id)
	.order("created_at", { ascending: false })
	.limit(15);

console.log("\nPending for Velveeta after:", after ?? 0);
...
console.log("\n=== Velveeta Room pilot done (draft mode) ===");
