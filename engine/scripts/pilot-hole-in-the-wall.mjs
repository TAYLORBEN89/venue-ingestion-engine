/**
 * Hole in the Wall pilot — Prekindle widget on /shows
 * Walkthrough: list times → promo link → flyer/RSVP/description → next list row
 *
 * Usage: node scripts/pilot-hole-in-the-wall.mjs [--probe-only]
 */
...
const SLUG = "hole-in-the-wall";
const CALENDAR = "https://www.theholeinthewallaustin.com/shows";
const WORKER =
...
console.log("\n=== Hole in the Wall pilot (Prekindle) ===\n");
...
if (vErr || !venue) {
	const { data: alt } = await sb
		.from("venues")
		.select("id, slug, name")
		.or("slug.ilike.%hole%,name.ilike.%hole%in%the%wall%")
		.limit(8);
...
await sb
	...
	.update({
		...
		website_url: venue.website_url || "https://www.theholeinthewallaustin.com/",
...
if (sourceId) {
	...
	if (error) {
		...
		console.log("Updated source", sourceId, "→ auto / draft / 90d");
	} else {
		console.log("Updated source", sourceId, "→ prekindle / draft / 90d");
	}
} else {
...
const { count: before } = await sb
	...
	.eq("venue_id", venue.id);
console.log("\nPending for HITW before:", before ?? 0);
...
console.log("\nPending for HITW after:", after ?? 0);
...
console.log("\n=== Hole in the Wall pilot done (draft mode) ===");
